module PORR2.Perceptron.Sequential (
...
nextWeights _xpos _xneg _w  = 
    let xpos = _xpos $| ("i","t")
        ...
        w = _w $| ("","i")
        ypos = rsgn `Multilinear.map` (w * xpos) -- y $| ("","t")
        yneg = rsgn `Multilinear.map` (w * xneg) -- y $| ("","t")
...
learnBinaryPerceptron trainImages' trainLabels' (pos,neg) learnIters =
    ...
    let trainPositiveImages = Multilinear.filterIndex "t" 
            (\i -> (trainLabels' $$| ("t",[i])) == Scalar (fromIntegral pos) ) trainImages'
        trainNegativeImages = Multilinear.filterIndex "t" 
...
testBinaryPerceptron p testImages' testLabels' (pos,neg) = 
    let -- from all MNIST images and labels, filter only required  
        positiveImages = Multilinear.filterIndex "t" 
            (\i -> (testLabels' $$| ("t",[i])) == Scalar (fromIntegral pos) ) testImages'
        negativeImages = Multilinear.filterIndex "t" 
        ...
        ypos = sgn `Multilinear.Generic.map` (p $| ("","j") * positiveImages $| ("j","t"))
        yneg = sgn `Multilinear.Generic.map` (p $| ("","j") * negativeImages $| ("j","t"))
